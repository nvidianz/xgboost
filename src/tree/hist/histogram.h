...
class HistogramPolicyContainer : public BuildPolicy {
 ...
 public:
  ...
  template <typename TreeT>
  void SyncHistogram(TreeT const &tree, std::vector<bst_node_t> const &nodes_to_build,
                     std::vector<bst_node_t> const &nodes_to_trick) {
    ...
    SubtractHistParallel(ctx_, space, tree, nodes_to_build, nodes_to_trick, &buffer_, &hist_);
  }
    ...
    std::visit(
        [&](auto &&target_builders) {
          ...
          for (bst_target_t t{0}; t < n_targets; ++t) {
            target_builders[t].AddHistRows(tree, &nodes, &dummy_sub, false);
          }
          ...
          for (bst_target_t t = 0; t < n_targets; ++t) {
            target_builders[t].SyncHistogram(tree, nodes, dummy_sub);
          }
        },
        target_builders_);
  }
  ...
  void BuildHistLeftRight(Context const *ctx, DMatrix *p_fmat, TreeView const &tree,
    ...
    std::visit(
        [&](auto &&target_builders) {
          ...
          target_builders.front().AddHistRows(tree, &nodes_to_build, &nodes_to_sub, true);
          ...
          for (bst_target_t t = 1; t < target_builders.size(); ++t) {
            target_builders[t].AddHistRows(tree, &nodes_to_build, &nodes_to_sub, false);
          }
          ...
          for (bst_target_t t = 0; t < n_targets; ++t) {
            target_builders[t].SyncHistogram(tree, nodes_to_build, nodes_to_sub);
          }
        },
        target_builders_);
  }
